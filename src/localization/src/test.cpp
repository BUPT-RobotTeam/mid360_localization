...
{
public:
    OctomapProc(const std::string &topic, const double resolution, const float z_min, const float z_max, const std::string &publish_topic = "/debug/octomap") 
        : octree_(resolution), resolution_(resolution), z_max(z_max), z_min(z_min)
    {
    ...
    void Octomap2Mat(cv::Mat &dst, cv::Point &origin)
    {
        mutex_.lock();

        // 获取octomap的空间边界
        double x_min, x_max, y_min, y_max;
        double z_min_tmp, z_max_tmp; //后续用不到
        octree_.getMetricMin(x_min, y_min, z_min_tmp); 
        octree_.getMetricMax(x_max, y_max, z_max_tmp);

        // 设置二维图像的大小，根据分辨率确定
        int width = static_cast<int>((x_max - x_min)/resolution_);
        int height = static_cast<int>((y_max - y_min)/resolution_);
        ...
        for (octomap::OcTree::leaf_iterator it = octree_.begin_leafs(), end = octree_.end_leafs(); it != end; ++it) {
            ...
            // 将 3D 坐标投影到 2D（XY平面）
            int x_pixel = static_cast<int>((pt.x() - x_min)/resolution_);
            int y_pixel = static_cast<int>((pt.y() - y_min)/resolution_);
        ...
        mutex_.unlock();
        origin = cv::Point(static_cast<int>(-x_min/resolution_), static_cast<int>(y_min));
        map2D.copyTo(dst);
    ...
    //         octomap::point3d pt = it.getCoordinate();
    //         // 将3D坐标转换为2D栅格坐标
    //         int x_pixel = static_cast<int>((pt.x() - x_min));
    //         int y_pixel = static_cast<int>((pt.y() - y_min));
    //         // 判断栅格是否在有效范围内
...
int main(int argc, char **argv)
{
    ...
    int detect_duration = 2;
    float z_max, z_min;
    if (!nh.getParam("z_max", z_max)){
        ROS_WARN("Failed to get param 'z_max'");
    }
    if (!nh.getParam("z_min", z_min)){
        ROS_WARN("Failed to get param 'z_min'");
    }
    if (!nh.getParam("resolution", resolution)){
        ROS_ERROR("Failed to get param 'resolution'.");
    }
    if (!nh.getParam("detect_duration", detect_duration)){
        ROS_ERROR("Failed to get param 'detect_resolution'.");
    }
    
    OctomapProc pcl2octomap("/cloud_registered", resolution, z_min, z_max);
    ros::spin();
